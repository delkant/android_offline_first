package com.rodrigodelcanto.mobile.commons.util;
...
public class ImageUtil {
     ...
     * @param reqWidth  The requested width of the resulting bitmap
     * @param reqHeight The requested height of the resulting bitmap
     * @return The description to be used for inSampleSize
     */
    public static int calculateInSampleSize(BitmapFactory.Options options,
        ...
        if (height > reqHeight || width > reqWidth) {
            ...
            // Calculate the largest inSampleSize description that is a power of 2 and keeps both
    ...
    public static Bitmap fontAwesomeToBitmap(Context context, String fontAwesomeIconName) {
        return fontAwesomeToBitmap(context, fontAwesomeIconName, 0);
    }

    public static Bitmap fontAwesomeToBitmap(Context context, String fontAwesomeIconName, int color) {
        FontAwesome.Icon icon = FontAwesome.Icon.valueOf(fontAwesomeIconName);
        return fontAwesomeToBitmap(context, icon, color);
    }

    public static Bitmap fontAwesomeToBitmap(Context context, FontAwesome.Icon icon) {
        return fontAwesomeToBitmap(context, icon, 0);
    }

    public static Bitmap fontAwesomeToBitmap(Context context, FontAwesome.Icon icon, int color) {
        color = color == 0 ? R.color.white : color;
        Bitmap bitmapIcon = null;
        if (icon != null) {
            bitmapIcon = fontAwesomeToDrawable(context, icon, color).toBitmap();
        }

        return bitmapIcon;
    }

    public static IconicsDrawable fontAwesomeToDrawable(Context context, String fontAwesomeIconName, int color) {
        FontAwesome.Icon icon = FontAwesome.Icon.valueOf(fontAwesomeIconName);
        return fontAwesomeToDrawable(context, icon, color);
    }
    public static IconicsDrawable fontAwesomeToDrawable(Context context, FontAwesome.Icon icon, int color){
        color = color == 0 ? R.color.white : color;
        return new IconicsDrawable(context).icon(icon).color(context.getResources().getColor(color));
    }
}
